...
Fig1S <- vennDiagram(v_count) #3604 shared between sites - CORE?

#######################################################################################
#MENA network analysis 
MENA_ZP <- read.csv('Data/ZiPi_bean_0.88_new.csv', header = T)
core_US_all <- append(US_occ_1, core_ITS)
...
#   mutate(otu=ID) %>%
#   mutate(core=if_else(otu %in% core_US_all, 'US core (n=75 OTUs)', 'no (n=497 OTUs)')) %>%
#   left_join(tax_v1[,c('Phylum', "Order","Genus", 'otu')], by='otu') %>%
#   filter(core=='US core (n=75 OTUs)') %>%
#   group_by(otu, Order,Genus) %>%
#   summarise(otuZi=max(Pi)) %>%
#   arrange(desc(otuZi))

# netw_prop <- read.csv('network_analysis/global_network_properties_mena.csv', header=T)
# netw_prop <- read.csv('network_analysis/whole_network_node_attributes.csv', header=T)
# netw_prop <- read.csv('network_analysis/all_node_attributes_cytoscape.csv', header=T)

modules_No <- MENA_ZP
names(modules_No)[2] <- 'otu'

(module_plot_data <- data.frame(otu=as.factor(rownames(otu.rhizo.rel.abun)),otu.rhizo.rel.abun) %>%
    gather(sample_ID, abun, -otu) %>%
    ...
    theme(legend.position = 'none',
          ...
          strip.background = element_rect(colour="transparent", fill="transparent")))
setEPS()
postscript('network_analysis/MENA_modules.eps', width=10, height = 2.5)
module_plot_data
dev.off()
...
mena_int<- read.csv('network_analysis/interaction_mena.csv', sep=',', header=F)
head(mena_int)
...
# summarise(n_core_core=sum(core_core),
#           n_bact_fung=sum(bact_fung),
#           n_bact_arch=sum(arch_fung),
#           n_arch_fun=sum(arch_fung),
#           all=sum(n_bact_fung, n_bact_arch, n_arch_fun))
...
bulk_rhizo_core_taxa <- data.frame(otu=as.factor(rownames(otu.rel.abun)),otu.rel.abun) %>%
  gather(sample_ID, abun, -otu) %>%
  filter(otu %in% core_US_otus) %>%
  left_join(map_combined[,c('sample_ID','pH', 'state', 'bean', 'plot', 'site', 'soil')], by='sample_ID') %>%
  left_join(tax_v1[,c('Kingdom','Phylum','Family' ,"Genus", 'otu', 'final_names')], by='otu') %>%
  group_by(site,sample_ID, soil, otu) %>%
  summarise(rel_abun=sum(abun))%>%
  #           n_samples=length(sample_ID),
  #            rel_abun=sum_abun/n_samples) %>%
  #filter(site == 'CO')%>%
  ggplot(aes(x=soil, y=rel_abun, color=soil)) +
...
us_rhizo_bulk_core <- data.frame(otu=as.factor(rownames(otu.rel.abun)),otu.rel.abun) %>%
  gather(sample_ID, abun, -otu) %>%
  filter(otu %in% core_US_otus) %>%
  left_join(map_combined[,c('sample_ID','pH', 'state', 'bean', 'plot', 'site', 'soil')], by='sample_ID') %>%
  left_join(tax_v1[,c('Kingdom','Phylum','Family' ,"Genus", 'otu', 'final_names')], by='otu') %>%
  group_by(site, soil, otu, sample_ID) %>%
  summarise(rel_abun=sum(abun)/length(sample_ID))

ggplot(us_rhizo_bulk_core,aes(y = rel_abun, x = soil, fill = soil)) +
  geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .8) +
  geom_point(aes(y = rel_abun, color = soil), position = position_jitter(width = .15), size = .5, alpha = 0.8) +
  geom_boxplot(width = .1, guides = FALSE, outlier.shape = NA, alpha = 0.5,position = position_nudge(x = 0.3)) +
...
data.frame(otu=as.factor(rownames(otu.rel.abun)),otu.rel.abun) %>%
  ...
  group_by(site, soil, otu, sample_ID) %>%
  summarise(rel_abun=sum(abun)/length(sample_ID)) %>%
  ggplot(aes(y = rel_abun, x = soil, fill = site)) +
  geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .8) +
  geom_point(aes(y = rel_abun, color = site), position = position_jitter(width = .15), size = .5, alpha = 0.8) +
  #geom_boxplot(width = .1, guides = FALSE, outlier.shape = NA, alpha = 0.5,position = position_nudge(x = 0.3)) +
  guides(fill = FALSE) +
  guides(color = FALSE) +
...
col.otu.rel.abun <- decostand(otu_col_rare, method="total", MARGIN=2) #calculating relative abundance
map_col$SampleID <- as.character(map_col$SampleID)
map_col$Accession <- as.character(map_col$Accession)
map_col$Group <- as.character(map_col$Group)
map_col$SampleID <- as.character(map_col$SampleID)
map_col$Sample_ID <- rownames(map_col)
map_col_v2 <- map_col
rownames(map_col_v2) <- NULL

col_rhizo_bulk_core <- data.frame(otu=as.factor(rownames(col.otu.rel.abun)),col.otu.rel.abun) %>%
  gather(Sample_ID, abun, -otu) %>%
  filter(otu %in% global_core) %>%
  left_join(map_col_v2[,c('Accession','Group', 'SampleID', 'Sample_ID')], by='Sample_ID') %>%
  mutate(soil = if_else(Accession == 'Bulk', 'bulk', 'rhizosphere')) %>%
  group_by(Group, soil, otu) %>%
  summarise(rel_abun=sum(abun)/length(Sample_ID),
            sample_No=length(Sample_ID)) 

ggplot(col_rhizo_bulk_core, aes(y = rel_abun, x = soil, fill = Group)) +
  geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .8) +
  geom_point(aes(y = rel_abun, color = Group), position = position_jitter(width = .15), size = .5, alpha = 0.8) +
  geom_boxplot(width = .1, guides = FALSE, outlier.shape = NA, alpha = 0.5,position = position_nudge(x = 0.3)) +
  #geom_errorbar(data = sumld, aes(ymin = lower, ymax = upper, y = mean), position = position_nudge(x = 0.3), width = 0) +
  #expand_limits(x = 5.25) +
...
bartlett.test(rel_abun~soil, data = us_rhizo_bulk_core)
tidy(aov(rel_abun~soil*site, data=us_rhizo_bulk_core))
stat_us <- data.frame(us_rhizo_bulk_core) %>%
  do(tidy(TukeyHSD(aov(.$rel_abun~.$soil*.$site))))

bartlett.test(rel_abun~soil, data = col_rhizo_bulk_core)
tidy(aov(rel_abun~soil*Group, data=col_rhizo_bulk_core))
stat_col <- data.frame(col_rhizo_bulk_core) %>%
  do(tidy(TukeyHSD(aov(.$rel_abun~.$soil*.$Group))))

#****************************************************************************
#ZOTUs
######
zotu_us <- read.table('US/ZOTU_filtered_table.txt', sep='\t', row.names = 1, header=T)
map_combined <- read.table('map_v2.txt', row.names = 1, sep='\t', header=T)
...
#comparing matching ZOTUs to reads within the core OTUs
#using reads.FromFasta.py script to get the reads matching the core OTUs
US_fastq <- read.table("US/OTU_fastq_map.txt", sep='\t')
head(US_fastq)
...
ggplot(No.reads.subOTU,aes(x=V10, fill=V4, y=rel.abun)) +
  geom_bar(stat ='identity', color='black') +
  labs(y='Relative read number of subOTU per OTU', x='OTUs at 97% identity')+
  theme_classic()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  #geom_text(aes(),position=position_fill(vjust=0.5)) +
  geom_text(data=No.reads.subOTU, aes(y=rel.abun,label=V4), position=position_fill(vjust=0.5))
...
BLAST.coreReads <- read.table('blastCORE.txt')
...
matchTable <- left_join(BLAST.coreReads, US_fastq) %>%
  tidyr::separate(readID, c('sample_ID', 'read.No'), '[.]', remove = F) %>%
  left_join(map_v2[,c('state','soil', 'sample_ID')], by='sample_ID') %>%
  filter(soil=='rhizosphere') %>%
  group_by(refID, uniqueRead) %>%
  summarise(nReads=length(readID)) %>%
  group_by(refID) %>%
  mutate(nUnique=length(uniqueRead),
         sumReads=sum(nReads),
         rel.No.reads=nReads/sumReads) %>%
  arrange(desc(rel.No.reads))
...
# ggplot(aes(x=name, y=relCount, fill=variable))+
# geom_bar(stat = 'identity') +
# theme_classic() +
# theme(axis.text.x = element_text(angle=90, hjust=1, vjust = .5)) 
#     
# setEPS()
# postscript('subOTU.eps', width=6, height = 5)
# matchTable %>%
#   #mutate(name=paste(refID, ' (',nUnique,')',sep='')) %>%
#   filter(uniqueRead %in% as.character(subOTUsiteSpecific$uniqueRead)) %>%
#   left_join(subOTUoccupancy) %>%
#   group_by(name) %>%
#   summarise(nUnique=unique(nUnique),
#             maxRelNo=max(rel.No.reads)) %>%
#   mutate(name=fct_reorder(name, -maxRelNo)) %>%
#   ggplot(aes(x=name,y=maxRelNo)) +
#   geom_point(pch = 21) +
#   ylim(0,1)+
#   labs(x='Core OTUs', y='Relative read number of max(subOTU)')+
#   theme_classic() +
#   theme(axis.text.x = element_text(angle=90, hjust=1, vjust = .5)) 
# dev.off()


COREsubOTUtable <- read.table("ZOTU_subOTU_UNOISE_table.txt", header = T, row.names = 1)
COREsubOTUmap <- read.table("ZOTUS_subOTU_UNOISE_map.txt", header=F)
...
# HQ597858.1.1508 (Acidobacterium) is taxon which does not have any ZOTU that is consistently detected.

ggplot(plotData,aes(name_new, maxRel))+
  geom_point(size=2) +
  labs(x=NULL, y=NULL)+
  theme_bw()+
