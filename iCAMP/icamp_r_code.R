...
wd="~/Documents/git/PAPER_Stopnisek_2019_BeanBiogeography/iCAMP"
wd="~/iCAMP"
...
save.wd="~/Documents/git/PAPER_Stopnisek_2019_BeanBiogeography/iCAMP"
save.wd="~/iCAMP"
...
rand.time=1  # randomization time, 1000 is usually enough. For example test, you may set as 100 or less to save time.
nworker=28 # nworker is thread number for parallel computing, which depends on the CPU core number of your computer.
memory.G=200 # to set the memory size as you need (but should be less than the available space in your hard disk), so that calculation of large tree will not be limited by physical memory. unit is Gb.
...
bin.size.limit = 32 # setting can be changed to explore the best choice. # here set as 5 just for the small example dataset. For real data, usually try 12 to 48.
